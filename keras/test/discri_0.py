...
model.compile(loss="categorical_crossentropy",
              optimizer=sgd,
              metrics=["accuracy"])
#plot_model(model,"discri_0.png")

##========== fitting ==========##
csvlogger = CSVLogger("discri_0_cross_val.csv")
board = TensorBoard(log_dir="log")
start = time.time()
model.fit([anode[int(0.1*sample_size):sample_size],cathode[int(0.1*sample_size):sample_size]],
          ans[int(0.1*sample_size):sample_size],
          epochs=epochs,
          batch_size=batch_size,
          validation_data=[[anode[:int(0.1*sample_size)],cathode[:int(0.1*sample_size)]],ans[:int(0.1*sample_size)]],
          callbacks=[csvlogger,board])
end = time.time()
print("Fitting time is {} second.".format(end-start))
model.save("discri_0_cross_val.h5")

start = time.time()
pred = model.predict([anode[:int(0.1*sample_size)],cathode[:int(0.1*sample_size)]])
end = time.time()
np.savetxt("discri_0_cross_val.dat",pred)
