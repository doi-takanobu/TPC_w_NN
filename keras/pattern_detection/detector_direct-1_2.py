...
dx = np.sqrt((xs[:,0:1]-xv[:,0:1])*(xs[:,0:1]-xv[:,0:1])+(xs[:,2:3]-xv[:,2:3])*(xs[:,2:3]-xv[:,2:3]))
dx_test = np.sqrt((xs_test[:,0:1]-xv_test[:,0:1])*(xs_test[:,0:1]-xv_test[:,0:1])+(xs_test[:,2:3]-xv_test[:,2:3])*(xs_test[:,2:3]-xv_test[:,2:3]))
...
Output_thr = Dense(1,activation="relu")(y7)
...
csvlogger = CSVLogger("direct-1_2.csv")
cb = [csvlogger]
if TENSOR_BOARD==1:
    board = TensorBoard(log_dir="log_direct",histogram_freq=1)
    cb.append(board)
start = time.time()
model.fit(cell,[dx,thr],validation_data=[cell_test,[dx_test,thr_test]]
         ,epochs=1500,batch_size=128,callbacks=cb)
end = time.time()
print("Fitting time: ",end-start,"s")

model.save("direct-1_2.h5")

print("begin prediction")
pred_dx,pred_thr = model.predict(cell_test)
np.savetxt("direct-1_2.dat",np.concatenate((pred_dx,pred_thr),axis=1))
