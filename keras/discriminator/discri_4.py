...
##========== fitting ==========##
csvlogger = CSVLogger("discri_4.csv")
#board = TensorBoard(log_dir="log")
start = time.time()
model.fit([anode[:sample_size],cathode[:sample_size]],
          ans[:sample_size],
          epochs=epochs,
...
model.save("discri_4.h5")

start = time.time()
pred = model.predict([anode[int(0.9*sample_size):],cathode[int(0.9*sample_size):]])
end = time.time()
np.savetxt("discri_4.dat",pred)
