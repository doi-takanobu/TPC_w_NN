...
from keras.layers import Input,Conv2D,MaxPooling2D,SeparableConv2D
...
csvlogger = CSVLogger("indirect_bipass.csv")
#board = TensorBoard(log_dir="./log/",histogram_freq=1)

start = time.time()
model.fit([cell[:,0:1],cell[:,1:2]],point/10.,epochs=500,batch_size=64,
          validation_data=[[cell_test[:,0:1],cell_test[:,1:2]],point_test/10.],
          callbacks=[csvlogger])
end = time.time()
print("Learning time is {} second".format(end-start))
model.summary()
model.save("indirect_bipass.h5")

pred = model.predict([cell_test[:,0:1],cell_test[:,1:2]])
np.savetxt("indirect_bipass.dat",pred*10.,header="avs avc aes aec cvs cvc ces cec [pixel]")
