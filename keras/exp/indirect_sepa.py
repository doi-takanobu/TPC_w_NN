...
y = Conv2D(filters=8,kernel_size=3,padding="same",
...
csvlogger = CSVLogger("indirect_sepa.csv")
#board = TensorBoard(log_dir="./log/",histogram_freq=1)

start = time.time()
model.fit([cell[:,0:1],cell[:,1:2]],point,epochs=500,batch_size=64,
          validation_data=[[cell_test[:,0:1],cell_test[:,1:2]],point_test],
          callbacks=[csvlogger])
end = time.time()
print("Learning time is {} second".format(end-start))
model.summary()
model.save("indirect_sepa.h5")

pred = model.predict([cell_test[:,0:1],cell_test[:,1:2]])
np.savetxt("indirect_sepa.dat",pred,header="avs avc aes aec cvs cvc ces cec [pixel]")
