...
csvlogger = CSVLogger("direct_norm-4.csv")
cb = [csvlogger]
if TENSOR_BOARD==1:
...
model.save("direct_norm-4.h5")

print("begin prediction")
start = time.time()
pred = model.predict([cell_test[:,0:1],cell_test[:,1:2]])
end = time.time()
np.savetxt("direct_norm-4.dat",(pred)*factor,header="dx theta phi")
