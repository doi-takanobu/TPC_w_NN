...
csvlogger = CSVLogger("indirect_norm-5.csv")
...
model.summary()
model.save("indirect_norm-5.h5")

pred = model.predict([cell_test[:,0:1],cell_test[:,1:2]])
np.savetxt("indirect_norm-5.dat",pred*factor,header="avs avc aes aec cvs cvc ces cec [pixel]")
