...
x3 = Conv2D(filters=64,kernel_size=(4,4),padding="same",activation="relu",data_format="channels_first")(x2)
y3 = Conv2D(filters=64,kernel_size=(4,4),padding="same",activation="relu",data_format="channels_first")(y2)
x4 = MaxPooling2D(pool_size=(4,4),data_format="channels_first")(x3)
y4 = MaxPooling2D(pool_size=(4,4),data_format="channels_first")(y3)
x5 = Conv2D(filters=256,kernel_size=(4,4),padding="same",activation="relu",data_format="channels_first")(x4)
y5 = Conv2D(filters=256,kernel_size=(4,4),padding="same",activation="relu",data_format="channels_first")(y4)
...
csvlogger = CSVLogger("direct_norm-5.csv")
cb = [csvlogger]
if TENSOR_BOARD==1:
...
model.save("direct_norm-5.h5")

print("begin prediction")
start = time.time()
pred = model.predict([cell_test[:,0:1],cell_test[:,1:2]])
end = time.time()
np.savetxt("direct_norm-5.dat",(pred)*factor,header="dx theta phi")
