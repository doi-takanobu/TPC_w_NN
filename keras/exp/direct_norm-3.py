...
x7 = Flatten()(x4)
y7 = Flatten()(y4)
x8 = Dense(128,activation="sigmoid")(x7)
y8 = Dense(128,activation="sigmoid")(y7)
x9 = Dropout(0.5)(x8)
y9 = Dropout(0.5)(y8)
x10 = concatenate([x9,y9],axis=1)
x11 = Dense(64,activation="sigmoid")(x10)
x12 = Dropout(0.5)(x11)
x13 = Dense(16,activation="sigmoid")(x12)
y13 = Dense(16,activation="sigmoid")(x12)
x14 = Dropout(0.5)(x13)
y14 = Dropout(0.5)(y13)
Output_dx = Dense(1,activation="relu")(x14)
Output_thr = Dense(2,activation="relu")(y14)

model = Model(inputs=[Input_a,Input_c],outputs=[Output_dx,Output_thr])
#sgd = SGD(lr=0.01,momentum=0.9,decay=1.e-6,nesterov=True)
#model.compile(optimizer=sgd,loss="mse")
#plot_model(model,"direct_exp.png")
model.compile(optimizer="Adadelta",loss="mse")

csvlogger = CSVLogger("direct_norm-3.csv")
cb = [csvlogger]
if TENSOR_BOARD==1:
    board = TensorBoard(log_dir="log_direct",histogram_freq=1)
    cb.append(board)
factor = np.array([90.,180.])#half of full range
start = time.time()
model.fit([cell[:,0:1],cell[:,1:2]],[dx/100.,np.concatenate((thr,phi),axis=1)/factor],
          validation_data=[[cell_test[:,0:1],cell_test[:,1:2]],[dx_test/100.,np.concatenate((thr_test,phi_test),axis=1)/factor]]
         ,epochs=500,batch_size=64,callbacks=cb)
end = time.time()
print("Fitting time: ",end-start,"s")

model.save("direct_norm-3.h5")

print("begin prediction")
start = time.time()
pred_dx,pred_thr = model.predict([cell_test[:,0:1],cell_test[:,1:2]])
end = time.time()
np.savetxt("direct_norm-3.dat",
           np.concatenate((pred_dx*100.,pred_thr*factor),axis=1),
           header="dx theta phi")
