...
csvlogger = CSVLogger("indirect_norm-3.csv")
...
start = time.time()
model.fit([cell[:,0:1],cell[:,1:2]],point/factor-0.5,epochs=500,batch_size=64,
          validation_data=[[cell_test[:,0:1],cell_test[:,1:2]],point_test/factor-0.5],
          callbacks=[csvlogger])
end = time.time()
print("Learning time is {} second".format(end-start))
model.summary()
model.save("indirect_norm-3.h5")

pred = model.predict([cell_test[:,0:1],cell_test[:,1:2]])
np.savetxt("indirect_norm-3.dat",(pred+0.5)*factor,header="avs avc aes aec cvs cvc ces cec [pixel]")
